...
const Sidebar = ({
  ...
  const onDrop = (acceptedFiles) => {
    ...
    const validFiles = acceptedFiles.filter((file) => {
      if (isDerivedArtifact(file.name)) {
        console.warn(
          `Rejected derived artifact: ${file.name}`
        );
        return false;
      }
    ...
    <>
      <aside
        style={{
          ...
          transition:
            'width 0.22s cubic-bezier(0.16, 1, 0.3, 1), min-width 0.22s cubic-bezier(0.16, 1, 0.3, 1), background-color 0.2s ease',
          height: '100vh',
          zIndex: 10,
          position: 'relative'
        }}
      >
        ...
        <div
          style={{
            padding:
              '0.85rem 0.75rem 0.4rem 0.75rem'
          }}
        >
          <button
            onClick={onNewChat}
